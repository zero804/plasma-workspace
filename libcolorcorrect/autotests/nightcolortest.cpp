...
void TestNightColor::testStageData_data()
{
    ...
    QTest::newRow("noChange") << true << 0 << DEFAULT_NIGHT_TEMPERATURE << 0. << 0. << QTime(6, 0, 0) << QTime(18, 0, 0) << FALLBACK_SLOW_UPDATE_TIME << false
                              << false << false;
    ...
    QTest::newRow("deactivate+temperature") << false << 0 << 1000 << 0. << 0. << QTime(6, 0, 0) << QTime(18, 0, 0) << FALLBACK_SLOW_UPDATE_TIME << true << true
                                            << true;
    QTest::newRow("location+differentMode") << true << 2 << 0 << 0. << 0. << QTime(6, 0, 0) << QTime(18, 0, 0) << FALLBACK_SLOW_UPDATE_TIME << true << true
                                            << true;
    QTest::newRow("time+defaultMode") << true << 0 << DEFAULT_NIGHT_TEMPERATURE << 0. << 0. << QTime(10, 0, 0) << QTime(20, 0, 0) << 1 << false << true << true;
    QTest::newRow("location+mode") << true << 1 << DEFAULT_NIGHT_TEMPERATURE << 50. << -20. << QTime(6, 0, 0) << QTime(18, 0, 0) << FALLBACK_SLOW_UPDATE_TIME
                                   << true << true << true;
