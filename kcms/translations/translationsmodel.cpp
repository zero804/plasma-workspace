    ...
    void start()
    {
        ...
        connect(transaction, &PackageKit::Transaction::finished, this, [this](PackageKit::Transaction::Exit status, uint code) {
            ...
            if (m_packageIDs.size() != m_packages.size()) {
                qWarning() << "Not all missing packages managed to resolve!" << m_packages << m_packageIDs;
            }
            install();
        });
    }
...
inline CompletionCheck *makeCompletionCheck(Args&&... __args)
{
    if (KOSRelease().idLike().contains("ubuntu")) {
        return new UbuntuCompletionCheck(std::forward<Args>(__args)...);
    }
    return nullptr;
}
...
{
    CompletionCheck *check = makeCompletionCheck(languageCode, this);
    if (!check) {
        return; // no checking support - default to assume complete
    }
    connect(check, &CompletionCheck::finished, this, [this, languageCode](CompletionCheck::Result result, const QStringList &missingPackages) {
        sender()->deleteLater();
        ...
        case CompletionCheck::Result::Error:
            qWarning() << "Failed to get completion status for" << languageCode;
            return;
        case CompletionCheck::Result::Incomplete: {
            ...
            if (changed) {
                dataChanged(createIndex(index, 0), createIndex(index, 0), {IsIncomplete});
            }
            return;
        }
        case CompletionCheck::Result::Complete:
            if (m_incompleteLanguagesWithPackages.contains(languageCode)) {
                m_incompleteLanguagesWithPackages.remove(languageCode);
                dataChanged(createIndex(index, 0), createIndex(index, 0), {IsIncomplete});
            }
            return;
        }
...
void SelectedTranslationsModel::completeLanguage(int index)
{
    ...
    connect(completer, &LanguageCompleter::complete, this, [this, code] {
        ...
        dataChanged(createIndex(index, 0), createIndex(index, 0), {IsInstalling});
    });
    m_incompleteLanguagesWithPackages.remove(code);
    m_installingLanguages << code;
    completer->start();
    dataChanged(createIndex(index, 0), createIndex(index, 0), {IsIncomplete, IsInstalling});
#else
    Q_UNUSED(index);
#endif
}
